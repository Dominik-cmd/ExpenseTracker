...
import { ApiService, QueueStatus } from '../../core/services/api.service';

const EMPTY_STATUS: QueueStatus = { pendingCount: 0, pending: [], recentlyProcessed: [] };
...
export class QueueComponent implements OnInit {
  ...
  protected readonly status = signal<QueueStatus>(EMPTY_STATUS);

  ngOnInit(): void {
    interval(POLL_INTERVAL_MS).pipe(
      startWith(0),
      switchMap(() => this.api.getQueueStatus()),
  ...
  protected refresh(): void {
    this.api.getQueueStatus().pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (data) => this.status.set(data),
