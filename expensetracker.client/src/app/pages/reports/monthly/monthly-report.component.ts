...
import { EMPTY_MONTHLY_REPORT } from '../../../core/constants/fallbacks';
import { MonthlyReport, NarrativeResponse } from '../../../core/models';
...
@Component({
  ...
  imports: [CommonModule, FormsModule, CardModule, InputTextModule, TableModule, NgxEchartsDirective, AppCurrencyPipe],
  template: `
      ...
      <p-card header="Monthly report" subheader="Trends, category shifts, and merchant concentration for the selected month.">
        <div class="flex flex-column md:flex-row gap-3 align-items-end">
          ...
          <div class="grid flex-1">
            <div class="col-12 md:col-3"><div class="metric-label">Total</div><div class="metric-value">{{ report().totalAmount | appCurrency }}</div></div>
            <div class="col-12 md:col-3"><div class="metric-label">Previous month</div><div class="metric-value">{{ report().previousMonthTotal | appCurrency }}</div></div>
            <div class="col-12 md:col-3"><div class="metric-label">Change</div><div class="metric-value">{{ report().percentChange }}%</div></div>
            <div class="col-12 md:col-3"><div class="metric-label">Rolling 3m avg</div><div class="metric-value">{{ report().rolling3MonthAverage | appCurrency }}</div></div>
          </div>
        </div>
      </p-card>
            ...
            <div class="table-scroll-wrapper">
              <p-table [value]="report().categoryComparisons" responsiveLayout="scroll">
                ...
                <ng-template pTemplate="body" let-item>
                  <tr>
                    <td>{{ item.name }}</td>
                    <td>{{ item.currentAmount | appCurrency }}</td>
                    <td>{{ item.previousAmount | appCurrency }}</td>
        ...
        <p-table [value]="report().topMerchants" responsiveLayout="scroll">
          ...
          <ng-template pTemplate="body" let-merchant>
            <tr>
              ...
              <td>{{ merchant.totalAmount | appCurrency }}</td>
            </tr>
          </ng-template>
        </p-table>
      </p-card>
...
export class MonthlyReportComponent {
  ...
  protected readonly selectedMonth = signal(`${EMPTY_MONTHLY_REPORT.year}-${String(EMPTY_MONTHLY_REPORT.month).padStart(2, '0')}`);
  protected readonly report = signal<MonthlyReport>(EMPTY_MONTHLY_REPORT);
  ...
  constructor() {
    this.loadReport(EMPTY_MONTHLY_REPORT.year, EMPTY_MONTHLY_REPORT.month);
  }
  ...
  private loadReport(year: number, month: number): void {
    forkJoin({
      report: this.analyticsService.getMonthlyAnalytics(year, month).pipe(
        catchError(() => of({ ...EMPTY_MONTHLY_REPORT, year, month }))
      ),
      narrative: this.analyticsService.getMonthlyNarrative(year, month).pipe(
        catchError(() => of(null))
      )
    }).pipe(
