...
import { InvestmentProvider } from '../../../core/models';
      ...
      @if (manualProvider(); as manual) {
        <p-card>
          ...
          <div class="provider-body">
            ...
            <div class="status-line">
              {{ manualAccountCount() }} manual account(s) · Total value {{ manualTotal() | appCurrency:'1.0-0' }}
...
export class InvestmentSettingsComponent {
  ...
  private loadProviders(): void {
    forkJoin({
      providers: this.investmentService.getProviders().pipe(catchError(() => of<InvestmentProvider[]>([]))),
  ...
  protected saveIbkrConfig(): void {
    ...
    this.investmentService.updateProvider(ibkr.id, payload)
  ...
  protected testIbkr(): void {
    ...
    this.investmentService.testProvider(ibkr.id)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (result) => {
  ...
  protected toggleIbkr(enabled: boolean): void {
    ...
    const operation = enabled ? this.investmentService.enableProvider(ibkr.id) : this.investmentService.disableProvider(ibkr.id);
    operation.pipe(takeUntilDestroyed(this.destroyRef)).subscribe(() => this.loadProviders());
  }

  protected syncNow(): void {
    this.syncing.set(true);
    this.investmentService.triggerSync()
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: () => {
