...
import { EMPTY_WEBHOOK_SETTINGS } from '../../../core/constants/fallbacks';
      ...
      <div class="col-12 xl:col-7">
        <p-card header="Allowed SMS senders" subheader="Trusted sender aliases used by the SMS webhook and parser.">
          <div class="flex flex-column gap-3">
            ...
            @if (senders.length > 0) {
              <div class="flex flex-wrap gap-2">
                @for (sender of senders; track sender; let i = $index) {
              ...
              <small class="text-color-secondary">No senders configured. Add at least one trusted sender.</small>
            }
...
export class WebhookSettingsComponent {
  ...
  protected readonly settings = signal<WebhookSettings>(EMPTY_WEBHOOK_SETTINGS);
  protected readonly rotatingSecret = signal(false);
  protected readonly savingSenders = signal(false);
  protected senders: string[] = [...EMPTY_WEBHOOK_SETTINGS.senders];
  protected newSender = '';
  ...
  protected rotateSecret(): void {
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        this.settingsService.rotateWebhookSecret().pipe(
          ...
          error: () => {
            this.messageService.add({ severity: 'error', summary: 'Rotation failed', detail: 'Could not rotate the webhook secret. Please try again.' });
          }
        });
      }
    });
  ...
  protected saveSenders(): void {
    ...
    this.settingsService.updateSmsSenders(this.senders).pipe(
      ...
      error: () => {
        this.messageService.add({ severity: 'error', summary: 'Save failed', detail: 'Could not save the sender list. Please try again.' });
      }
    });
  }
  ...
  protected loadSettings(): void {
    this.settingsService.getWebhookSettings().pipe(
      catchError(() => of(EMPTY_WEBHOOK_SETTINGS)),
      takeUntilDestroyed(this.destroyRef)
    ).subscribe((settings) => {
