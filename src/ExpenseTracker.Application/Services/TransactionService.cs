...
public sealed class TransactionService(
  ...
  IAuditLogRepository auditLogRepository) : ITransactionService
{
  ...
  {
    if (!await categoryRepository.ExistsAsync(request.CategoryId, ct))
    {
    ...
    Guid id, Guid userId, UpdateTransactionRequest request, CancellationToken ct)
  {
    ...
    {
      if (!await categoryRepository.ExistsAsync(request.CategoryId.Value, ct))
      {
        throw new InvalidOperationException("Category does not exist.");
      }
