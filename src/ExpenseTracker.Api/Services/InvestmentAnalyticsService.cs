...
public sealed class InvestmentAnalyticsService(AppDbContext dbContext)
{
    public async Task<PortfolioSummaryDto> GetSummaryAsync(Guid userId, CancellationToken ct)
    {
        ...
        var ytdStartTotal = await dbContext.PortfolioHistories
          .Where(h => h.Account.UserId == userId && h.SnapshotDate >= yearStart)
          .OrderBy(h => h.SnapshotDate)
          .Select(h => (decimal?)h.MarketValue)
          .FirstOrDefaultAsync(ct);

        if (ytdStartTotal.HasValue)
        {
            ytdStartTotal = await dbContext.PortfolioHistories
    ...
    public async Task<List<AccountSummaryDto>> GetAccountsAsync(Guid userId, CancellationToken ct)
    {
        var accounts = await dbContext.InvestmentAccounts
          ...
          .Include(a => a.ManualBalance)
          .Where(a => a.UserId == userId && a.IsActive)
          .OrderByDescending(a => a.Holdings.Sum(h => h.MarketValue)
            + (a.ManualBalance != null ? a.ManualBalance.Balance : 0))
          .ToListAsync(ct);
    ...
    public async Task<List<HoldingDto>> GetHoldingsAsync(Guid userId, CancellationToken ct)
    {
        return await dbContext.Holdings
          ...
          .Select(h => new HoldingDto(
            h.Id, h.Account.DisplayName, h.Instrument.Symbol, h.Instrument.Name,
            h.Instrument.AssetClass, h.Quantity, h.CostBasisPerShare, h.MarkPrice,
            h.MarketValue, h.UnrealizedPnl, h.UnrealizedPnlPercent, h.Currency))
          .ToListAsync(ct);
    }

    public async Task<AllocationBreakdownDto> GetAllocationAsync(
      Guid userId, string allocationType, CancellationToken ct)
    {
        var accounts = await dbContext.InvestmentAccounts
          .AsNoTracking()
          .Include(a => a.Provider)
    ...
    public async Task<List<HistoryPointDto>> GetHistoryAsync(
      ...
      Guid userId, int limit, CancellationToken ct)
    {
        var ibkrTxns = await dbContext.InvestmentTransactions
          ...
          .Select(t => new RecentActivityDto(
            ...
            t.AccountId, t.Account.DisplayName, "ibkr",
            t.TransactionType,
            t.Description ?? (t.TransactionType + " " + t.Instrument!.Symbol),
            t.NetAmount, t.Currency, t.Quantity,
        ...
        var manualUpdates = await dbContext.ManualBalanceHistories
          ...
          .Select(h => new RecentActivityDto(
            ...
            h.AccountId, h.Account.DisplayName, "manual",
            "BALANCE_UPDATE",
            "Updated " + h.Account.DisplayName + ": " + h.Currency + h.Balance
              + (h.Note != null ? " (" + h.Note + ")" : ""),
            h.Balance, h.Currency, null, null))
          .ToListAsync(ct);
...
internal record AllocationItemDto(string Label, decimal Value, string Currency);
