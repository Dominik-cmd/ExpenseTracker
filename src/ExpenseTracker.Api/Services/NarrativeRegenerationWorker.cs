  ...
  protected override async Task ExecuteAsync(CancellationToken stoppingToken)
  {
    await foreach (var request in channel.Reader.ReadAllAsync(stoppingToken))
    {
      try
      {
        ...
        {
          case "dashboard":
            await narrativeService.RegenerateDashboardNarrativeAsync(
              request.UserId, force: false, stoppingToken);
            break;
          case "monthly" when request.Year.HasValue && request.Month.HasValue:
            await narrativeService.RegenerateMonthlyNarrativeAsync(
              request.UserId, request.Year.Value, request.Month.Value, force: false, stoppingToken);
            break;
          case "yearly" when request.Year.HasValue:
            await narrativeService.RegenerateYearlyNarrativeAsync(
              request.UserId, request.Year.Value, force: false, stoppingToken);
            break;
          case "investments":
            await narrativeService.RegenerateInvestmentNarrativeAsync(
              request.UserId, force: false, stoppingToken);
            break;
        }
      }
...
public record NarrativeRegenerationRequest(
  Guid UserId, string Type, string Scope, int? Year = null, int? Month = null);
