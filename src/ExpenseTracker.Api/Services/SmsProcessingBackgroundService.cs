  ...
  ILogger<SmsProcessingBackgroundService> logger) : BackgroundService
{
  ...
  {
    logger.LogInformation("SmsProcessingBackgroundService started");
    ...
    logger.LogInformation("SmsProcessingBackgroundService stopping");
  }
  ...
  private async Task RequeuePendingMessagesAsync(CancellationToken ct)
  {
    ...
    {
      logger.LogInformation("Requeuing {Count} pending SMS messages for processing", pendingIds.Count);
    }
  ...
  private async Task ProcessMessageAsync(Guid messageId, CancellationToken ct)
  {
    ...
    try
    {
      ...
      logger.LogInformation(
        "SMS processed: {MessageId}, merchant {Merchant}, amount {Amount}, category {Category} ({Source})",
        messageId, parsed.MerchantNormalized, parsed.Amount, resolution.Category.Name, resolution.Source);
    }
    catch (Exception ex)
    {
      rawMessage.ParseStatus = ParseStatus.Failed;
  ...
  private static async Task<CategoryResolution> ResolveCategoryAsync(
    AppDbContext dbContext, ILlmProviderResolver providerResolver,
    ParsedSms parsed, Guid userId, CancellationToken ct)
  {
    ...
    if (provider is not null && providerConfiguration is not null)
    {
      ...
      var result = await provider.CategorizeAsync(
        ...
        new CategorizationRequest(
          parsed.MerchantRaw, parsed.MerchantNormalized,
          parsed.Amount, parsed.Direction, parsed.TransactionType, parsed.Notes)
        { UserId = userId },
        categories,
        ct);
      ...
      if (result is not null)
      {
        ...
        {
          var resolution = new CategoryResolution(
            category, CategorySource.Llm,
    ...
    var fallback = await dbContext.Categories
      .FirstAsync(x => x.UserId == userId && x.Name == "Uncategorized" && x.ParentCategoryId == null, ct);
  ...
  private static async Task<CategoryResolution> ApplyMiscIncomeRedirectAsync(
    AppDbContext dbContext, CategoryResolution resolution,
    ParsedSms parsed, Guid userId, CancellationToken ct)
  {
    ...
    var miscIncome = await dbContext.Categories
      .FirstOrDefaultAsync(
  ...
  private static async Task<Category?> ResolveCategoryResultAsync(
    AppDbContext dbContext, CategorizationResult result, Guid userId, CancellationToken ct)
  {
    var parent = await dbContext.Categories
      .FirstOrDefaultAsync(x => x.UserId == userId && x.Name == result.Category && x.ParentCategoryId == null, ct);
    if (parent is null)
    {
      return null;
    }
    ...
    var child = await dbContext.Categories
      .FirstOrDefaultAsync(x => x.UserId == userId && x.ParentCategoryId == parent.Id && x.Name == result.Subcategory, ct);
  ...
  private static async Task UpsertMerchantRuleAsync(
    AppDbContext dbContext, string merchantNormalized,
    Guid categoryId, string createdBy, Guid userId, CancellationToken ct)
  {
    var rule = await dbContext.MerchantRules
